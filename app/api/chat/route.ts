...
// Generate city regex with normalized and escaped city names
const allCities = Object.values(governoratesWithCities)
  .flat()
  .map((city) => normalizeArabicText(city)); // Normalize each city name
...
export async function POST(req: Request) {
  try {
    ...
    console.log("Normalized text for regex:", normalizedText); // Debug: Log normalized text
    const cityMatch = normalizedText.match(cityRegex);
    if (cityMatch) {
      clientCity = cityMatch[2]; // Capture the city name (group 2)
      console.log(`Detected client city: ${clientCity}`);
      // Map normalized city back to original city name for filtering
