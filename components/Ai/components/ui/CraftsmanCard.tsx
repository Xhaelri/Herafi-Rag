...
export const CraftsmanCard: React.FC<Craftsman> = ({
  id,
  sourceId,
  name,
  craft,
  ...
  console.log(`CraftsmanCard: ${name}, id: ${id}, sourceId: ${sourceId}`); // Debug log
    ...
    <div className="bg-white flex flex-col justify-between p-4 border border-[#C0392B] rounded-2xl shadow-md hover:shadow-md transition-all duration-300 transform hover:-translate-y-1">
      <div className="flex items-start justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h3 className="text-lg font-aboreto text-[#8D5524]">{name}</h3>
            <p className="text-sm font-inter text-[#8D5524]">{craft}</p>
          </div>
        </div>
      ...
      <div className="mt-5 flex items-center justify-between">
        ...
        <a
          ...
          target="_blank"
        >
          <span className="text-[#8D5524] hover:text-[#C0392B]">
            عرض الملف
          </span>
        </a>
      </div>
